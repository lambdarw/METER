...
class QueryGenerator:
    ...
    def objects_extract(self, claim_list):
        ...
        message = [[
            {"role": "model", "parts": [{"text": self.prompt[self.type]["object"]["system"]}]},
    ...
    def get_response(self, claim_list, type):
        ...
        self.message_list = [
            [
                {"role": "model", "parts":[{"text": self.prompt[type]["attribute"]["system"]}]},
                {"role": "user", "parts":[{"text": self.prompt[type]["attribute"]["user"].format(objects=objects,claims=claim_list)}]}
            ],
            [
                {"role": "model", "parts":[{"text": self.prompt[type]["scene-text"]["system"]}]},
                {"role": "user", "parts":[{"text": self.prompt[type]["scene-text"]["user"].format(claims=claim_list)}]}
            ],
            [
                {"role": "model", "parts":[{"text": self.prompt[type]["fact"]["system"]}]},
                {"role": "user", "parts":[{"text": self.prompt[type]["fact"]["user"].format(claims=claim_list)}]}
            ],
        ]
        # self.message_list = [
